...
const GUIDE_TEXT := """[color=#f2d9a0][b]欢迎回到温馨小屋～[/b][/color]

[b]基础操作[/b]
- 左键点击地板/家具：点击处会出现一个吸引物（爱心/小鱼干等，设置里可换），{name}停顿、头顶冒「！」后被吸引过去，到达后物品消失；到达家具旁会自动触发交互
- 点击「电脑」：打开仿电脑系统（游戏选择 / 五子棋 / 聊天 / 日记）
- 点击「冰箱 / 厨具 / 饭桌」：拿食物 → 做饭 → 吃饭（沙发也能吃；没食物时先到冰箱拿）
- 点击「布置台」：自定义家具位置（长按拾起 → 点击放下）
- {name}偶尔会走神（冒「？」），好感度越高越愿意回应你的引导；她也会自己活动（饿了吃饭、累了休息、无聊玩电脑——玩电脑时可以点「一起玩」加入）

[b]界面[/b]
- 右上角「设置」：通用 / 显示 / 音频 / AI 设置（配置 API 后聊天才能用）
- 「设置」下方「手机」：随时给{name}发消息，她会回复、冒气泡，还可能被指令吸引去家具旁
- 右侧「家具介绍」：随时查看每件家具的功能
- 左侧立绘：{name}（当前角色，会随着聊天次数记录记忆）
...
func setup_guide(char_name: String = "梅尔") -> void:
	title_label.text = "新手攻略"
	content_label.text = GUIDE_TEXT.replace("{name}", char_name)
